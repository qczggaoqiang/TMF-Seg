...
from ldm.data.MosMedData import MosMedDataTest
...
def main():
    ...
    parser.add_argument("--ckpt", type=str, default="logs/2026-01-22T21-43-24_experiment_name_MosMedData/checkpoints/epoch=736-step=401999.ckpt",
    ...
    if opt.dataset == "synapse-b":
        ...
        run = "2025-12-30T14-55-36_experiment_name"
        print("Evaluate on kseg dataset in binary segmentation manner.")
        opt.config = glob.glob(os.path.join("logs", run, "configs", "*-project.yaml"))[0]
        #opt.ckpt = f"logs/{run}/checkpoints/model.ckpt"
        opt.outdir = "outputs/slice2seg-samples-kseg"
        dataset = KSEGTest()
    elif opt.dataset == "MosMedData":
        run = "2026-01-22T21-43-24_experiment_name_MosMedData"
        print("Evaluate on MosMedData dataset in binary segmentation manner.")
        opt.config = glob.glob(os.path.join("logs", run, "configs", "*-project.yaml"))[0]
        #opt.ckpt = f"logs/{run}/checkpoints/model.ckpt"
        opt.outdir = "outputs/slice2seg-samples-MosMedData"
        dataset = MosMedDataTest()
    else:
    ...
    for idx in range(opt.times):
        ...
        if image_seg_label is not None:
            output_path = "outputs/slice2seg-samples-MosMedDataTest/0"  # 设定保存的路径
