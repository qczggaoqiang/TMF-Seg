...
class SDSeg(LatentDiffusion):
    def __init__(self, first_stage_config, cond_stage_config, load_only_unet=True, num_classes=2, text_stage_key="text", use_text_conditioning=False, *args, **kwargs):
    ...
    def on_train_batch_start(self, batch, batch_idx, dataloader_idx):
        ...
        if self.current_epoch == 0 and self.global_step == 0 and batch_idx == 0:  # and not self.restarted_from_ckpt:
            ...
            print(f"### USING TEXT CONDITIONING: \033[31m{self.use_text_conditioning}\033[0m ###")
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ['caption', 'coordinates_bbox']:
                    ...
                    xc = DDPM.get_input(self, batch, cond_key)  # ← 移除 .to(self.device)
            else:
                xc = x
            ...
            if self.use_text_conditioning:
                # 获取文本
                text = batch.get(self.text_stage_key, None)
                if text is not None and bs is not None:
                    text = text[:bs]  # List[str]不需要.to(device)
                ...
                if not self.cond_stage_trainable or force_c_encode:
                    if isinstance(xc, dict) or isinstance(xc, list):
                        ...
                        # 调用新的融合方法（这里统一to device）
                        c = self.get_learned_conditioning_with_text(xc.to(self.device), text)
                else:
                    # cond_stage_trainable=True，保存为元组，稍后在forward中处理
                    c = (xc, text)
            else:
                # 原始逻辑：不使用文本
                if not self.cond_stage_trainable or force_c_encode:
            ...
            if bs is not None:
                c = c[:bs] if not isinstance(c, tuple) else c
    ...
    def get_learned_conditioning_with_text(self, c_img, c_text):
        """获取图像+文本的融合条件编码

        Args:
            c_img: 图像 tensor [B, 3, 256, 256]
            c_text: 文本列表 List[str] 或 None
        ...
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
                # 调用ImageTextFusionEmbedder.encode(img, text)
                c = self.cond_stage_model.encode(c_img, c_text)
                if isinstance(c, DiagonalGaussianDistribution):
                    c = c.mode()
            else:
                # 直接调用forward
                c = self.cond_stage_model(c_img, c_text)
        else:
            assert hasattr(self.cond_stage_model, self.cond_stage_forward)
            c = getattr(self.cond_stage_model, self.cond_stage_forward)(c_img, c_text)
    ...
    def forward(self, x, c, cls_id, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            assert c is not None
                ...
                #c = self.get_learned_conditioning(c)
            if self.cond_stage_trainable:
                # 如果c是(image, text)元组（来自get_input），需要先编码
                if isinstance(c, tuple) and len(c) == 2:
                    c_img, c_text = c
                    c = self.get_learned_conditioning_with_text(c_img, c_text)
                else:
                    c = self.get_learned_conditioning(c)
            if self.shorten_cond_schedule:  # TODO: drop this option
                tc = self.cond_ids[t].to(self.device)
