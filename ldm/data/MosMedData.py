...
class MosMedData(Dataset):
    ...
    def __init__(self, train_dir, val_dir, test_dir, size=256, interpolation="nearest", mode=None, num_classes=2):
        ...
        print(f"[Dataset]: MosMedData+ with 2 classes, in {self.mode} mode")
    ...
    def __getitem__(self, i):
        ...
        segmentation = Image.fromarray(cv2.cvtColor(cv2.imread(example["file_path_"].replace("frames", "masks")), cv2.COLOR_BGR2RGB))
...
class MosMedDataTrain(MosMedData):
    def __init__(self, train_dir="data/MosMedData+/train/frames", **kwargs):
        super().__init__(train_dir=train_dir, val_dir=None, test_dir=None, mode="train", **kwargs)


class MosMedDataValidation(MosMedData):
    def __init__(self, val_dir="data/MosMedData+/val/frames", **kwargs):
        super().__init__(train_dir=None, val_dir=val_dir, test_dir=None, mode="val", **kwargs)


class MosMedDataTest(MosMedData):
    def __init__(self, test_dir="data/MosMedData+/test/frames", **kwargs):
